...
let _IS_SHIPPING_ON_ITEM_QUANTITIES = 1;
...
$(document).on('click', '._ecom_addtocart', function () {
         ...
         success: function (response) {
             ...
             if (response.has_inventory_enable && api_available_stock_count != -1) {
                 if (api_available_stock_count >= pd_quantity_val) {
                     ...
                     $('#_pd_variation_data ._pd_error_msg').html('');
                 } else {
                ...
                //  $('#_pd_addtocart_btn').removeClass('_disabled');
                $('#_pd_addtocart_btn').addClass('_disabled');
                $('#_pd_variation_data ._pd_error_msg').html('Out Of Stock');
             }
 ...
 $(document).ready(function () {
    ...
    _IS_SHIPPING_ON_ITEM_QUANTITIES = $('meta[name="_is_shipping_on_item_quantities"]').attr('content') ?? 1;

    // quantity update add-to-cart
    $('#_pd_quantity #pd-quantity-input').on('change paste keyup', function () {
        $('#_pd_addtocart_btn').attr('data-pquantity', $(this).val());
    });
    ...
    // show status on quenty update
    $('#pd-quantity-input').on('keyup', function () {
        if($(this).val() >= 1){
            updateProductDetails();
        }
    });
    // show status on quenty update
    $('#pd-quantity-input').on('change', function () {
        if($(this).val() >= 1){
            updateProductDetails();
        }
    });
 ...
 function makeObj($this) {
    // let q = parseFloat($('#pd-quantity-input').val() ?? $this.data('pquantity'));
    // if ($("#_pd_choices_area").length > 0) {
    //     q = parseFloat($('#pqv-pquantity').val() ?? $('#pd-quantity-input').val() ?? $this.data('pquantity'));
    // }
     var tempObj = {
         "p_id": $this.data('pid'),
 ...
 function makeProductCard(obj) {
     ...
     if (obj.p_hasdiscount) {
         var myCardString = '<div class="cc-product-row _parent_card_row _pcard_id_' + obj.p_id + '">' +
             ...
             '<button class="cp-remove-btn _do_cart_calc _remove_prod_btn_" data-prod_id="' + obj.p_id + '"><i class="far fa-trash-alt"></i>Â REMOVE</button>' +
             '</div>' +
             '</div>' +
             '</div>' +
             '</div>';
     ...
     if (getLS("wCart") != null) {
         ...
         if (myCartArray.length > 0) {
             for (var i = 0; i < myCartArray.length; i++) {
                 ...
                 if(_IS_SHIPPING_ON_ITEM_QUANTITIES == 0){
                    myCartArray[i].p_shippingtotalcost = shrinkFloat(prod_shippingcost);
                 }else{
                ...
                if(_IS_SHIPPING_ON_ITEM_QUANTITIES == 0){
                    myCartDetailsObj.cart_shippingcost += shrinkFloat(myCartArray[i].p_shippingcost);
                }else{
             ...
             onSuccess: function ($form) {
                 ...
                 $.ajax({
                     ...
                     success: function (response) {
                        ...
                        //  STRIPE_PAY
                         else if (response.order_status.payment_type === "STRIPE_PAY") {
                            // console.log('STRIPE_PAY log -> ');
                            // console.log(response);

                            // var redirectUrl = "/stripe/pay?website_id=" + website_id + "&ecom_order_id=" + response.order_status.ecom_order_id + "&_token=" + _token;
                            /*
                            var redirectUrl = "/stripe/pay?website_id=" + $('.__website_id').val() + "&payment_type=STRIPE_PAY" +
                            "&item_name=" + response.order_status.description +
                            "&item_name=" + response.order_status.description +
                            "&currency=" + response.order_status.currency +
                            "&amount=" + response.order_status.total_amt +
...
var regex = /^\/\w+\/thanks$/; // Regular expression pattern to match '/{random_id}/thanks'
if (regex.test(window.location.pathname)) {
    console.log('url thanks match');
    flushCartLS();
}
